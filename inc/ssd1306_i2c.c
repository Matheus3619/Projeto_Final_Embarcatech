...
void ssd1306_draw_char(uint8_t *buffer, int x, int y, char c) {
    if (c < 32 || c > 126) return; // Fora do intervalo da tabela
    
    uint16_t font_index = (c - 32) * 5; // Encontra o início do caractere na tabela
...
void ssd1306_draw_rect(uint8_t *ssd, uint8_t x, uint8_t y, uint8_t width, uint8_t height, bool set, bool fill) {
    // Desenha as linhas horizontais (superior e inferior)
    for (uint8_t i = x; i < x + width; i++) {
        ssd1306_set_pixel(ssd, i, y, set);               // Topo
        ssd1306_set_pixel(ssd, i, y + height - 1, set);  // Base
    }

    // Desenha as linhas verticais (esquerda e direita)
    for (uint8_t i = y; i < y + height; i++) {
        ssd1306_set_pixel(ssd, x, i, set);               // Esquerda
        ssd1306_set_pixel(ssd, x + width - 1, i, set);   // Direita
    }

    // Se fill for verdadeiro, preenche o conteúdo interno
    if (fill) {
        for (uint8_t i = x + 1; i < x + width - 1; i++) {
            for (uint8_t j = y + 1; j < y + height - 1; j++) {
                ssd1306_set_pixel(ssd, i, j, set);
            }
        }
    }
}
